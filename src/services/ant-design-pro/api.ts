...
export async function updateUser(data: Partial<API.CurrentUser>, options?: { [key: string]: any }) {
  try {
...
export async function deleteUser(userId: number, options?: { [key: string]: any }) {
  return request<API.BaseResponse<boolean>>(`/api/user/${userId}`, {
    method: 'DELETE',
    ...(options || {}),
  }).then(response => {
    console.log('Raw API response:', response); // 添加这行
